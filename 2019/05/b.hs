...
solve m pos res = 
    case toCommand $ show (m ! pos) of
        ...
        _:m1:"03" -> solve (M.insert (m ! (pos + 1)) 5 m) (pos + 2) res
        _:m1:"04" -> solve m (pos + 2) ((idx m1 (pos + 1) m):res)
        m2:m1:"05" -> 
            if idx m1 (pos + 1) m /= 0 
            then solve m (idx m2 (pos + 2) m) res
            else solve m (pos + 3) res
        m2:m1:"06" -> 
            if idx m1 (pos + 1) m == 0 
            then solve m (idx m2 (pos + 2) m) res
            else solve m (pos + 3) res
        m2:m1:"07" -> 
            ...
            then solve (M.insert (m ! (pos + 3)) 1 m) (pos + 4) res
            else solve (M.insert (m ! (pos + 3)) 0 m) (pos + 4) res
        m2:m1:"08" -> 
            if idx m1 (pos + 1) m == idx m2 (pos + 2) m 
            then solve (M.insert (m ! (pos + 3)) 1 m) (pos + 4) res
            else solve (M.insert (m ! (pos + 3)) 0 m) (pos + 4) res
