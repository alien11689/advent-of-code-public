...
weightsPerGroup = (weights.sum() * 0.25 ) as int
...
def selectSizes(){
    while(true){
        ...
        def third = pack(weights.findAll {!(it in (first + second))})
        def forth = pack(weights.findAll {!(it in (first + second + third))})
        def buckets = [first, second, third, forth].sort {it.size()}
