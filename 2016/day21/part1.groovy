def main(String input, String init) {
	input.split('\n')
		.collect {toRule(it)}
		.inject(init.toCharArray() as List){ cur, rule->
...
println(main(new File('sample.txt').text, 'abcde'))
println()
println(main(new File('input.txt').text, 'abcdefgh'))
